...
private struct TodayHeroCard: View {
    ...
    private func completionSummarySection(summary: TodayCompletionSummary) -> some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            if let nextActionText = summary.nextActionText(hasScheduledTemplate: dayPlan != nil || schedule?.template != nil) {
                Text(nextActionText)
    ...
    private struct TodayCompletionSummary {
        ...
        var subtitleText: String {
            ...
            if sessionCount == 1 {
                if totalDuration > 0 {
                    return "\(latestName) wrapped at \(completionTime) in \(formatDuration(totalDuration)). Dive back into the details or keep the momentum going."
                }
                return "\(latestName) is already in the books for today. Open the session details or jump into another round."
            }

            if totalDuration > 0 {
                return "\(sessionCount) sessions are logged today for \(formatDuration(totalDuration)) total. Open the day timeline or stack another session on top."
            }

            return "\(sessionCount) sessions are already logged today. Open the day timeline or stack another session on top."
        }
...
private struct FeaturedCompletedSessionCard: View {
    ...
    var body: some View {
        Button(action: action) {
            ...
            .padding(16)
            .background(.white.opacity(colorScheme == .dark ? 0.10 : 0.28), in: RoundedRectangle(cornerRadius: 22, style: .continuous))
            .overlay(
                RoundedRectangle(cornerRadius: 22, style: .continuous)
                    .stroke(.white.opacity(colorScheme == .dark ? 0.16 : 0.42), lineWidth: 1)
            )
        }
        .buttonStyle(.plain)
    }
